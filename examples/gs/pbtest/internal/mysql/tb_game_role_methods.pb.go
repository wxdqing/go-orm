...
// methods for pbtest.GameRole in mysql
...
func (m *GameRole) DecodeToContext(ctx Context, value proto.Message) error {
	...
	r.Level = m.Level
	r.Exp = m.Exp
	r.PlayerEnum = pbtest.CoveragePlayerEnum(m.PlayerEnum)
	r.Profile = m.Profile
	r.Version = m.Version
	if len(m.Heros) > 0 {
	...
	if m.Timestamps != nil {
		if r.Timestamps == nil {
			r.Timestamps = &pbtest.RoleTimestamps{}
		}
	...
	pbtest.SetGameRoleDefaults(value.(*pbtest.GameRole))
